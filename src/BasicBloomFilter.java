...
public class BasicBloomFilter implements Serializable {
  ...
  public static void main(String[] args) {
    BasicBloomFilter b = new BasicBloomFilter(0.01, 1000);
    int[] f = new int[] { 1, 2, 3 };

    int[] g = new int[] { 1, 2, 3 };
    b.add(int2byte(f));

    System.out.println(b.contains(int2byte(g)));
  ...
  private int numberOfAddedElements; // number of elements actually added to the
                                     // Bloom filter
  private int k; // number of hash functions

  static final Charset charset = Charset.forName("UTF-8"); // encoding used for
                                                           // storing hash
                                                           // values as strings

  static final String hashName = "MD5"; // MD5 gives good enough accuracy in
                                        // most circumstances. Change to SHA1 if
                                        // it's needed
   ...
   */
  public BasicBloomFilter(double c, int n, int k) {
   ...
   *          contain.
   */
  public BasicBloomFilter(int bitSetSize, int expectedNumberOElements) {
   ...
   */
  public BasicBloomFilter(double falsePositiveProbability,
   ...
   */
  public BasicBloomFilter(int bitSetSize, int expectedNumberOfFilterElements,
  ...
  public boolean equals(Object obj) {
    ...
    final BasicBloomFilter other = (BasicBloomFilter) obj;
   ...
   * of expected filter elements and the size of the Bloom filter. <br />
   * <br />
  ...
  public double getFalsePositiveProbability(double numberOfElements) {
    ...
    return Math
        .pow(
            (1 - Math.exp(-k * (double) numberOfElements / (double) bitSetSize)),
            k);
