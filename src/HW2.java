...
public class HW2 {
                              ...
                              for (int mx = -sumlx; mx <= sumlx; mx++) {
                                ...
                                for (int nx = -summx; nx <= summx; nx++) {
                                  ...
                                  if (res == null) {
                                    ...
                                    if (bf.contains(res.tapeArray())) {
                                      dupecounter++;
                                    } else {
                                      bfcounter++;
                                      bf.add(res.tapeArray());
                                      if (!bfwarning && bfcounter > bfsize) {
                                        bfwarning = true;
                                        System.out
                                            .println("=========BLOOM FILTER OVERFLOW==========");
                                      }
  ...
  public static void iterRecRel2(int maxsum) {
              ...
              for (int ex = -sumdx; ex <= sumdx; ex++) {
                ...
                for (int fx = -sumex; fx <= sumex; fx++) {
                  ...
                  if (fx != 0) {
                    byte lhmult = (byte) fx;
                    // for (int gx = -sumfx; gx <= sumfx; gx++) {
                    for (int gx = 1; gx <= 7; gx += 2) {
                      int sumgx = sumfx - Math.abs(gx);
                      for (int hx = -sumgx; hx <= sumgx; hx++) {
                        int sumhx = sumgx - Math.abs(hx);
                        for (int ix = -sumhx; ix <= sumhx; ix++) {
                          int sumix = sumhx - Math.abs(ix);
                          for (int jx = -sumix; jx <= sumix; jx++) {
                            int sumjx = sumix - Math.abs(jx);
                            byte[] deltas = { (byte) gx, (byte) hx, (byte) ix,
                                (byte) jx };
                            for (int kx = -sumjx; kx <= sumjx; kx++) {
                              int sumkx = sumjx - Math.abs(kx);
                              byte lhadd = (byte) kx;
                              for (int lx = -sumkx; lx <= sumkx; lx++) {
                                int sumlx = sumkx - Math.abs(lx);
                                byte postcon = (byte) lx;

                                // TapeState benchmark = evalRecRel(new
                                // byte[]{1,0,-1,-2},(byte)1,(byte)1,new
                                // byte[]{-3,-1,-1,-1},(byte)0,(byte)0);

                                // if (ax == 1 && bx == 0 && cx == -1 && dx ==
                                // -2
                                // && ex == 1 && fx == 1 && gx == -3
                                // && hx == -1 && ix == -1 && jx == -1
                                // && kx == 0 && lx == 0) {
                                // System.out
                                // .println("=========BENCHMARK CROSSED==========");
                                // }

                                // System.out.println(recRelToString(accum,
                                // precon, lhmult, deltas, lhadd, postcon));
                                TapeState res = evalRecRel(accum.clone(),
                                    precon, lhmult, deltas, lhadd, postcon);
                                // System.out.println(evalRecRel(accum.clone(),
                                // precon, lhmult, deltas, lhadd, postcon));
                                ...
                                if (res == null) {
                                  ...
                                  if (bf.contains(res.tapeArray())) {
                                    ...
                                    CommandPairList suffix = simplePathfinder(
                                        ...
                                        0, bestSuffixLength - maxsum + sumlx
                                            - 19);
                                    if (suffix == null) {
                                      suboptcounter++;
                                      ...
                                      System.out.println("len:"
                                          + (maxsum - sumlx) + " unique:"
                                          + bfcounter + " dupe:" + dupecounter
                                          + " inf:" + infloopcounter
                                          + " subopt:" + suboptcounter);
                                      bestSuffixLength = bestResult.lengthSum;
                                    }
                                  }
                                }
